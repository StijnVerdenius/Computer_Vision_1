...
for jj = 1 : length(gaborFilterBank)
    real_filter = gaborFilterBank(jj).filterPairs(:, :, 1); % \\TODO: filter the grayscale input with real part of the Gabor
    imag_filter = gaborFilterBank(jj).filterPairs(:, :, 2); % \\TODO: filter the grayscale input with imaginary part of the Gabor
...
if smoothingFlag
    ...
    %END_FOR
    smoothed_featureMags =  cell(length(gaborFilterBank),1);
    for jj = 1:length(featureMags)
        smoothed_featureMags{jj} = imgaussfilt(featureMags{jj});
        features(:,:,jj) = smoothed_featureMags{jj};
    end
else
