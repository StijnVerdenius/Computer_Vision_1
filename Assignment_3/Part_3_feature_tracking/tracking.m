...
for index = 1:numel(images)-1
    ...
    [Vxs,Vys] = phillip(rgb2gray(grayImg1), rgb2gray(grayImg2), false, 15);
    ...
    for in = 1:numel(vecX)
        ...
        vecY(in) = Vxs(y_location, x_location).*multiplication;
        vecX(in) = Vys(y_location, x_location).*multiplication;
    end
